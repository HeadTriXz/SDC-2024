...
from lane_assist.image_manipulation.dynamic_calibrate.utils import get_transformed_corners


def warp_image(image: np.ndarray, matrix: np.ndarray, max_height: int = None) -> tuple[np.ndarray, int]:
    ...
    return cv2.warpPerspective(image, adjusted_matrix, (width, height), flags=cv2.INTER_NEAREST), cropped
